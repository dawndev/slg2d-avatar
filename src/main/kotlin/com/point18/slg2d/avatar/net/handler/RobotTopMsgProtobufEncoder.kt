    ...
    override fun encode(ctx: ChannelHandlerContext, msg: C2SMsg, out: MutableList<Any>) {
        ...
        if (datas == null) {
            ...
            if (protobufMsg != null) {  // 消息MessageLite可以是空的
                ...
                val wrappedBuffer = Unpooled.wrappedBuffer(intByte, msgNoByte, datas)  // 将编码后的消息编号，消息次号，消息主题打包
                out.add(wrappedBuffer)  //  放进输出消息队列

            } else {
                out.add(Unpooled.wrappedBuffer(intByte, msgNoByte))  //  放进输出消息队列
            }

        } else {
            out.add(Unpooled.wrappedBuffer(intByte, msgNoByte, datas))  //  放进输出消息队列
        }
